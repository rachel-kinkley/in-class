...
let svg = documnet.querySelector('avg');
...
let rect = document.createElem(NS, 'rect');
rect.setAttribute('x', 0);
rect.setAttribute('y', 0);
rect.setAttribute('width', WIDTH);
rect.setAttribute('height', HEIGHT);
rect.setAttribute('fill', FILL);
rect.style.pointEvents = 'none';
...
let button = document.createElem('button');
...
function renderCircle(coord, parent) {
    ...
    circle.style.fill = `rgb(0, ${cord.x}, ${cord.y})`;
    // Append
...
function update() {
    ...
    clearCirlces(container);
    drawCircles(state.circles, container);
    updateText(state.cirlces);
}
...
function updateText(data) {
    ...
    text.innerHTML = `There are ${data.length}, the averagee color is rgb(0, ${green}, ${blue}, their size is ${state.size})`;
